...
const themeLogic = `  const currentTheme = portalSettings?.theme || 'Dark';
  const ts = {
    ...
    Cyberpunk: {
      bg: 'bg-yellow-400', cardBg: 'bg-black', card: 'bg-black border-2 border-pink-500 shadow-[8px_8px_0px_0px_rgba(236,72,153,1)]', cardHover: 'hover:translate-x-1 hover:-translate-y-1 hover:shadow-[12px_12px_0px_0px_rgba(236,72,153,1)] transition-all',
  ...
  }[currentTheme as keyof typeof ts] || {
...
content = content.replace(/const currentTheme[\s\S]*?selection:bg-emerald-500\/30' \};/m, themeLogic);

// Ensure the outer wrapper respects the font family
// Line 102: <div className={`flex flex-col min-h-[100dvh] w-full ${ts.bg} font-sans ${ts.text} ${ts.selection}`}>
content = content.replace(/font-sans/g, '${ts.font}');
content = content.replace(/rounded-2xl/g, '${ts.roundedLg}');
content = content.replace(/rounded-\[2rem\]/g, '${ts.roundedXl}');
content = content.replace(/shadow-2xl shadow-black\/50/g, '${ts.shadow}');
