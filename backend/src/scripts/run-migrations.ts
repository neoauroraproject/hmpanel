...
async function main() {
  ...
  try {
    ...
    if (existingMigration && existingMigration.status === 'SUCCESS') {
      console.log(`[MigrationOrchestrator] Migration ${MIGRATION_VERSION} already executed successfully. Skipping.`);
      process.exit(0);
    }

    console.log(`[MigrationOrchestrator] Executing migration ${MIGRATION_VERSION}...`);
    ...
    console.log('[MigrationOrchestrator] Identity migration logic completed. Validating integrity...');
    ...
    console.log(`[MigrationOrchestrator] Migration ${MIGRATION_VERSION} finished successfully!`);
    process.exit(0);
  } catch (error: any) {
    console.error(`[MigrationOrchestrator] FAILED to execute migration ${MIGRATION_VERSION}:`, error);
