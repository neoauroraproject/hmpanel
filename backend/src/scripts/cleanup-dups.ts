...
async function main() {
  console.log(
    '[HMPanel] Checking for duplicate clients before schema migration...',
  );
  ...
  try {
    ...
    if (duplicates.length > 0) {
      console.log(
        `[HMPanel] Found ${duplicates.length} duplicate emails. Cleaning up...`,
      );
      ...
      for (const dup of duplicates) {
        ...
        for (const toDelete of clientsToDelete) {
          console.log(
            `[HMPanel] Deleting duplicate client ID ${toDelete.id} (email: ${toDelete.email}, inbounds: ${toDelete._count.inbounds})`,
          );
    ...
    console.error(
      '[HMPanel] Warning: Error during duplicate cleanup. This may be normal if the DB is uninitialized.',
      error.message,
    );
  } finally {
